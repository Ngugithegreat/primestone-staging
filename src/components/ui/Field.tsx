...
const CONTROL =
  "w-full rounded-none border border-white/10 bg-ink-900 px-3.5 text-[14px] text-white placeholder:text-slate-500 transition-all duration-200 outline-none focus:border-mint-500/60 focus:bg-ink-900 focus:shadow-[0_0_0_3px_rgba(207,166,83,0.12)] disabled:opacity-50";
...
export function Toggle({
  ...
  return (
    <button
      ...
      className="focus-ring flex w-full items-center justify-between gap-4 rounded-none border border-white/[0.07] bg-white/[0.02] px-4 py-3 text-left transition-colors hover:bg-white/[0.05]"
    >
...
export function SegmentedControl<T extends string>({
  ...
  return (
    <div
      className={cn(
        "inline-flex rounded-none border border-white/[0.07] bg-ink-900 p-1",
        className,
      )}
    >
      {options.map((o) => (
        <button
          ...
          className={cn(
            "focus-ring flex-1 rounded-none font-medium transition-all duration-200",
