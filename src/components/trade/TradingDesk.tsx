          ...
          <div className="border-b border-white/[0.06] p-3">
            <div className="relative">
              ...
              <input
                ...
                className="h-9 w-full rounded-none border border-white/[0.08] bg-ink-900 pl-9 pr-3 text-[13px] text-white outline-none transition-colors placeholder:text-slate-500 focus:border-mint-500/50"
              />
            </div>
            <div className="mt-2.5 flex flex-wrap gap-1.5">
              {CLASSES.map((c) => (
                <button
                  ...
                  onClick={() => setFilter(c)}
                  className={cn(
                    "rounded-none px-2 py-1 text-[11px] font-medium capitalize transition-colors",
                    filter === c
            ...
            {list.map((i) => {
              ...
              return (
                <div
                  key={i.id}
                  className={cn(
                    "group flex items-center gap-2 rounded-none px-2.5 py-2 transition-colors",
...
function CloseAllButton({ prices }: { prices: Record<string, number> }) {
  ...
  return (
    <button
      ...
      className="focus-ring rounded-none border border-rose-500/30 bg-rose-500/10 px-3 py-1.5 text-[12.5px] font-medium text-rose-400 transition-colors hover:bg-rose-500/20"
    >
      Close all
    </button>
  );
